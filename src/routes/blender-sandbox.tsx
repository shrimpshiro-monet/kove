...
export const Route = createFileRoute("/blender-sandbox")({
  component: BlenderSandboxPage,
});

export default function BlenderSandboxPage() {
  ...
  const [prompt, setPrompt] = useState("Direct Blender server-side rendering to add high-energy zoom-pulses and dynamic chromatic aberration.");
  ...
  const logMessage = (msg: string) => {
    setConsoleLogs(prev => [...prev, `[Blender Engine] ${msg}`]);
  };
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    formData.append("projectId", "blender-project-dev");
  ...
  const letGeminiDirect = async () => {
    ...
    setDirecting(true);
    logMessage(`Contacting live Gemini model to evaluate Blender headless scripting...`);
  ...
  const runRealEdits = async () => {
    if (!fileId) return;
    setEditing(true);
    logMessage(`Triggering real server-side engine render for Blender pipeline...`);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-white flex items-center gap-3">
              ...
              Blender Dedicated Engine Sandbox
            </h1>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card className="bg-slate-900 border-slate-800 text-slate-100">
            <CardHeader>
              ...
              <CardDescription>Upload clip for headless Blender rendering.</CardDescription>
            </CardHeader>
            <CardContent>
              <input
                type="file"
          ...
          <Card className="bg-slate-900 border-slate-800 text-slate-100">
            <CardHeader>
              ...
              <CardDescription>Instruct Gemini to direct Blender scripts.</CardDescription>
            </CardHeader>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="bg-slate-900 border-slate-800 text-slate-100">
            <CardHeader>
              <CardTitle className="text-lg">Blender Render Preview</CardTitle>
            </CardHeader>
