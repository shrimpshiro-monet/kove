...
export const Route = createFileRoute("/mlt-sandbox")({
  component: MltSandboxPage,
});

export default function MltSandboxPage() {
  ...
  const [prompt, setPrompt] = useState("Perform clean dynamic linking cuts and apply a vintage sepia filter overlay.");
  ...
  const logMessage = (msg: string) => {
    setConsoleLogs(prev => [...prev, `[MLT Engine] ${msg}`]);
  };
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    formData.append("projectId", "mlt-project-dev");
  ...
  const letGeminiDirect = async () => {
    ...
    setDirecting(true);
    logMessage(`Contacting live Gemini model to compile dynamic MLT XML blocks...`);
    ...
    try {
      ...
      logMessage(`Gemini successfully compiled compliant MLT XML blocks:`);
  ...
  const runRealEdits = async () => {
    if (!fileId) return;
    setEditing(true);
    logMessage(`Triggering real server-side engine render for MLT pipeline...`);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-white flex items-center gap-3">
              ...
              MLT Framework Dedicated Engine Sandbox
            </h1>
            <p className="text-slate-400 text-sm">
              LGPL-licensed broadcast-grade multimedia framework. Supports dynamic XML timeline composition & dynamic linking.
            </p>
          </div>
        </div>
        <Badge className="bg-blue-950 text-blue-300 border-blue-900">LGPL LICENSE</Badge>
      </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card className="bg-slate-900 border-slate-800 text-slate-100">
            <CardHeader>
              ...
              <CardDescription>Upload clip for MLT dynamic compositing.</CardDescription>
            </CardHeader>
            <CardContent>
              <input
                type="file"
          ...
          <Card className="bg-slate-900 border-slate-800 text-slate-100">
            <CardHeader>
              ...
              <CardDescription>Instruct Gemini to compile MLT XML.</CardDescription>
            </CardHeader>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="bg-slate-900 border-slate-800 text-slate-100">
            <CardHeader>
              <CardTitle className="text-lg">MLT Rendering Preview</CardTitle>
            </CardHeader>
            ...
            <CardContent>
              <div className="bg-slate-950 p-4 rounded border border-slate-800 font-mono text-xs text-slate-300 h-64 overflow-y-auto space-y-2">
                {consoleLogs.length === 0 ? (
                  <span className="text-slate-600">Console idle. Run MLT edits to view logs.</span>
                ) : (
