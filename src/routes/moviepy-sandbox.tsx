...
export const Route = createFileRoute("/moviepy-sandbox")({
  component: MoviePySandboxPage,
});

export default function MoviePySandboxPage() {
  ...
  const [prompt, setPrompt] = useState("Squeeze and trim the video clip to its first 5 seconds to highlight fast pacing.");
  ...
  const logMessage = (msg: string) => {
    setConsoleLogs(prev => [...prev, `[MoviePy Engine] ${msg}`]);
  };
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    formData.append("projectId", "moviepy-project-dev");
  ...
  const letGeminiDirect = async () => {
    ...
    setDirecting(true);
    logMessage(`Contacting live Gemini model to evaluate MoviePy script directives...`);
    ...
    try {
      ...
      logMessage(`Gemini evaluated contract & generated MoviePy code block:`);
  ...
  const runRealEdits = async () => {
    if (!fileId) return;
    setEditing(true);
    logMessage(`Triggering real server-side engine render for MoviePy pipeline...`);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-white flex items-center gap-3">
              ...
              MoviePy Dedicated Engine Sandbox
            </h1>
            <p className="text-slate-400 text-sm">
              MIT-licensed high-level Python rendering. Perfect for programmatic background cuts & automated renders.
            </p>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card className="bg-slate-900 border-slate-800 text-slate-100">
            <CardHeader>
              ...
              <CardDescription>Upload clip for script automation.</CardDescription>
            </CardHeader>
            <CardContent>
              <input
                type="file"
          ...
          <Card className="bg-slate-900 border-slate-800 text-slate-100">
            <CardHeader>
              ...
              <CardDescription>Instruct Gemini to direct MoviePy scripts.</CardDescription>
            </CardHeader>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="bg-slate-900 border-slate-800 text-slate-100">
            <CardHeader>
              <CardTitle className="text-lg">Script Processing Preview</CardTitle>
            </CardHeader>
            ...
            <CardContent>
              <div className="bg-slate-950 p-4 rounded border border-slate-800 font-mono text-xs text-slate-300 h-64 overflow-y-auto space-y-2">
                {consoleLogs.length === 0 ? (
                  <span className="text-slate-600">Console idle. Run script edits to view logs.</span>
                ) : (
