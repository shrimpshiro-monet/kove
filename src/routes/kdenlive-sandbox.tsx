...
export const Route = createFileRoute("/kdenlive-sandbox")({
  component: KdenliveSandboxPage,
});

export default function KdenliveSandboxPage() {
  ...
  const [prompt, setPrompt] = useState("Direct Kdenlive server-side rendering to apply a cinematic color grade with high-impact color vignette.");
  ...
  const logMessage = (msg: string) => {
    setConsoleLogs(prev => [...prev, `[Kdenlive Engine] ${msg}`]);
  };
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    formData.append("projectId", "kdenlive-project-dev");
  ...
  const letGeminiDirect = async () => {
    ...
    setDirecting(true);
    logMessage(`Contacting live Gemini model to evaluate Kdenlive headless scripting...`);
  ...
  const runRealEdits = async () => {
    if (!fileId) return;
    setEditing(true);
    logMessage(`Triggering real server-side engine render for Kdenlive pipeline...`);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-white flex items-center gap-3">
              ...
              Kdenlive Dedicated Engine Sandbox
            </h1>
            <p className="text-slate-400 text-sm">
              GPL-licensed pro editor. Extends copyleft safety under the SaaS Loophole (headless server isolation).
            </p>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card className="bg-slate-900 border-slate-800 text-slate-100">
            <CardHeader>
              ...
              <CardDescription>Upload clip for headless server rendering.</CardDescription>
            </CardHeader>
            <CardContent>
              <input
                type="file"
          ...
          <Card className="bg-slate-900 border-slate-800 text-slate-100">
            <CardHeader>
              ...
              <CardDescription>Instruct Gemini to direct Kdenlive renders.</CardDescription>
            </CardHeader>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="bg-slate-900 border-slate-800 text-slate-100">
            <CardHeader>
              <CardTitle className="text-lg">Headless Render Preview</CardTitle>
            </CardHeader>
