...
export const Route = createFileRoute("/omniclip-sandbox")({
  component: OmniClipSandboxPage,
});

export default function OmniClipSandboxPage() {
  ...
  const logMessage = (msg: string) => {
    setConsoleLogs(prev => [...prev, `[OmniClip Engine] ${msg}`]);
  };
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    formData.append("projectId", "omniclip-project-dev");
    ...
    try {
      ...
      logMessage(`Metadata extracted - Resolution: ${data.metadata?.width}x${data.metadata?.height}, Duration: ${data.metadata?.duration}s, FPS: ${data.metadata?.fps}`);
    } catch (err: unknown) {
  ...
  const letGeminiDirect = async () => {
    ...
    setDirecting(true);
    logMessage(`Contacting live Gemini model to evaluate OmniClip capability contract...`);
  ...
  const runRealEdits = async () => {
    if (!fileId) return;
    setEditing(true);
    logMessage(`Triggering real server-side engine render for OmniClip pipeline...`);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-white flex items-center gap-3">
              ...
              OmniClip Dedicated Engine Sandbox
            </h1>
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card className="bg-slate-900 border-slate-800 text-slate-100">
            <CardHeader>
              ...
              <CardDescription>Upload footage directly into OmniClip workspace.</CardDescription>
            </CardHeader>
            <CardContent>
              <input
                type="file"
          ...
          <Card className="bg-slate-900 border-slate-800 text-slate-100">
            <CardHeader>
              ...
              <CardDescription>Direct OmniClip using natural language.</CardDescription>
            </CardHeader>
