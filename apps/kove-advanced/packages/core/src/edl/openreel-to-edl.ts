  ...
  const tracks = project.timeline.tracks.map((orTrack, index) => {
    ...
    const clips = orTrack.clips.map((orClip) => {
      ...
      return {
        ...
        audio: {
          gain: orClip.volume,
          fadeIn: orClip.audioEffects?.find((e) => e.type === "audio-fade")?.params?.fadeIn as number | undefined,
          fadeOut: orClip.audioEffects?.find((e) => e.type === "audio-fade")?.params?.fadeOut as number | undefined,
          pan: orClip.audioEffects?.find((e) => e.type === "audio-pan")?.params?.pan as number | undefined,
        },
        effects,
        meta: orClip.meta,
      };
