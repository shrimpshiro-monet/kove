...
import { SOCIAL_MEDIA_PRESETS, type SocialMediaCategory } from "@kove/core";
import { TooltipProvider } from "@kove/ui";
import { openReelProjectToMonetEDL } from "@monet/openreel-adapter";
...
function App() {
  ...
  // Load EDL from URL params (passed from Monet AI chat)
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const edlBase64 = urlParams.get("edl");
    const threadId = urlParams.get("threadId");

    if (edlBase64) {
      try {
        const edlJson = decodeURIComponent(escape(atob(edlBase64)));
        const edl = JSON.parse(edlJson);

        // Convert EDL to OpenReel project format
        const openReelProject = convertEDLToOpenReelProject(edl, []);

        // Create a new project with the EDL data
        createNewProject(`AI Edit — ${edl.id || "Imported"}`, {
          width: openReelProject.settings.width,
          height: openReelProject.settings.height,
          frameRate: openReelProject.settings.frameRate,
        });

        // Store threadId for "Back to AI" navigation
        if (threadId) {
          useProjectStore.setState({
            project: {
              ...useProjectStore.getState().project,
              meta: {
                ...useProjectStore.getState().project?.meta,
                threadId,
              },
            },
          });
        ...
        console.error("Failed to load EDL from URL:", err);
      }
    }
