  ...
  return (
    <div className="h-16 border-b border-border flex items-center px-6 justify-between bg-background shrink-0 z-30 relative">
      <div className="flex items-center gap-4">
        ...
        <ProjectSwitcher />
        {(params.threadId || project?.meta?.threadId) && (
          <>
            <div className="h-6 w-px bg-border hidden md:block" />
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  onClick={() => {
                    const threadIdParam = params.threadId || project?.meta?.threadId;
                    if (threadIdParam) {
                      const chatUrl = `${window.location.origin}/chat/${threadIdParam}`;
                ...
                >
                  <ArrowLeft size={14} />
                  <span className="hidden lg:block">Back to AI</span>
                </button>
              </TooltipTrigger>
              <TooltipContent>Return to Monet AI chat</TooltipContent>
            </Tooltip>
          </>
        )}
      </div>
