...
def call_nemotron(prompt: str, timeout: int = 60) -> Optional[list]:
    """Call LLM via Cerebras → Groq → NVIDIA NIM chain to get refinement actions."""
    from analyzers.llm_provider import call_text_llm

    content = call_text_llm(
        prompt,
        max_tokens=4096,
        system_prompt="You are a JSON-only API. Return ONLY a valid JSON array of actions. No explanation text, no markdown, no code fences.",
    )
    if not content:
        return None
    ...
    try:
        ...
        print(f"[monet_refine] LLM response parse failed: {e}", file=sys.stderr)
        return None
